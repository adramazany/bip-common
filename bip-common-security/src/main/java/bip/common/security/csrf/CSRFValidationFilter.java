package bip.common.security.csrf;
 ...
 *
 * @see "https://github.com/righettod/poc-csrf"
...
//    @Override
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
        ...
        LOG.debug(String.format("CSRFValidationFilter.doFilter: targetOrigin=%s, header.Origin=%s, header.Referer=%s, cookie.CSRF_TOKEN_NAME=%s, ",targetOrigin,source,httpReq.getHeader("Referer"),this.determineCookieName(httpReq)));
        ...
        if (httpReq.getCookies() != null) {
            ...
            for (Cookie c:httpReq.getCookies()) {
                if(c.getName().equalsIgnoreCase(csrfCookieExpectedName)){
                    tokenCookie = c;
                    break;
                }
            }
...
//    @Override
    public void init(FilterConfig filterConfig) throws ServletException {
        ...
        try {
            String param = filterConfig.getInitParameter(TARGET_ORIGIN_JVM_PARAM_NAME);
            if(isBlank(param)){
                param = System.getProperty(TARGET_ORIGIN_JVM_PARAM_NAME);
            }
            this.targetOrigin = new URL(param);
