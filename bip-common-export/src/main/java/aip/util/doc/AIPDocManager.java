...
public class AIPDocManager {
	...
	public static void printOrSaveDocument(HttpServletRequest request, HttpServletResponse response, String reqCode, BIPPrintParam printParam, BIPReportLST list){
		printOrSaveDocument(request, response, reqCode, printParam, list, list.getTitle());
	}
	public static void printOrSaveDocument(HttpServletRequest request, HttpServletResponse response,String reqCode,BIPPrintParam printParam,BIPReportLST list,String title){
		if("SaveHTML".equalsIgnoreCase(reqCode) 
				|| "SaveText".equalsIgnoreCase(reqCode)
				|| "SaveExcel".equalsIgnoreCase(reqCode)
				|| "SavePdf".equalsIgnoreCase(reqCode)
	...
	private String getObjectFieldValue(Object obj, String fieldName,int row, String emptyValue) throws Exception {
		...
		if("radif".equalsIgnoreCase(fieldName) || "~radif".equalsIgnoreCase(fieldName) ){
			...
			value=NVL.getString(BIPUtil.invokeGetter(obj, fieldName) ,emptyValue);
		}
		return value;
	}
	...
	public File writeHTML(AIPDocParam docParam) throws IOException,AIPException{
		File f = BIPUtil.createTempFile("aip-", ".html",getBasePath(),docParam.getWebUserParamUser());
	...
	public File writePdf(AIPDocParam docParam) throws IOException,AIPException{
		...
		File dest = BIPUtil.createTempFile("aip-", ".pdf",getBasePath(),docParam.getWebUserParamUser());
		Document doc = new Document();
		//doc.setPageSize( PageSize.A4.rotate() );
		try {

//			if(docParam.getPrintParam().getPageSize()!=null)doc.setPageSize(docParam.getPrintParam().getPageSize());
			Rectangle pageSizeRect = PageSize.getRectangle(docParam.getPrintParam().getPageSize());
			if(docParam.getPrintParam().getPageSize()!=null)doc.setPageSize( pageSizeRect);
...
//			}
	...
	public File writeExcel(AIPDocParam docParam) throws IOException,AIPException{
		...
		HSSFWorkbook wb = new HSSFWorkbook();
		 ...
		 File dest = BIPUtil.createTempFile("aip-", ".xls",getBasePath(),docParam.getWebUserParamUser());
	...
	public File writeText(AIPDocParam docParam) throws IOException,AIPException{
		File f = BIPUtil.createTempFile("aip-", ".txt",getBasePath(),docParam.getWebUserParamUser());
		FileOutputStream fout = new FileOutputStream(f);
