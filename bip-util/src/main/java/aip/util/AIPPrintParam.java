...
public class AIPPrintParam {
	...
	public long[][] getRequestStartsAndEnds(int pageSize,long totalRows){
		...
		for(int i=0;i<pages.length;i++){
			...
			long page = NVL.getLng(pair[0])-1;
			long startRow = (page<0?0:page)*pageSize+1;
			startsAndEnds[i][0]= startRow;
			if(pair.length>1){
				startsAndEnds[i][1]= startRow-1+NVL.getLng(pair[1])*pageSize;
			}else{
	...
	public void fillParameters(HttpServletRequest request,AIPPrintParam sessionParam ) {
		setPrintRange(NVL.getString(request.getParameter("printRange")
							,NVL.getString(sessionParam.getPrintRange())
				));
		setPrintPageCurrent(NVL.getInt(request.getParameter("printRange")
							,NVL.getInt(sessionParam.getPrintPageCurrent())
				));
		setPrintPageFrom(NVL.getInt(request.getParameter("printPageFrom")
							,NVL.getInt(sessionParam.getPrintPageFrom())
				));
		setPrintPageTo(NVL.getInt(request.getParameter("printPageTo")
							,NVL.getInt(sessionParam.getPrintPageTo())
				));
		setPrintPageCustom(NVL.getString(request.getParameter("printPageCustom")
							,NVL.getString(sessionParam.getPrintPageCustom())
				));

		setPrintPageSize( NVL.getString(request.getParameter("printPageSize")
							,NVL.getString(sessionParam.getPrintPageSize())
				));
		setLandscape( NVL.getBool(request.getParameter("printPageLandscape") 
							,NVL.getBool(sessionParam.isLandscape())
				));
	}
