...
//		}
	...
	String loadHeaderTitle() {
		...
		if(!NVL.isEmpty(getErrorName())){
			...
			error+=NVL.getString(BIPCommonWeb.getAttributeAnyScope(pageContext, getErrorName()));
		}
		if(!NVL.isEmpty(getSuccessName())){
			//success+=NVL.getString(pageContext.getAttribute(getSuccessName()));
			success+=NVL.getString(BIPCommonWeb.getAttributeAnyScope(pageContext, getSuccessName()));
		}
